...
export class OnboardingTasksController {
  ...
  // --- Subtasks: the checklist inside a task's popup ---
  // No @Roles() for the same reason as the completion endpoints above:
  // access is "the employee this onboarding belongs to" (plus HR, read
  // only), which is data-dependent rather than a fixed role.
  ...
  listSubtasks(
    ...
    return this.onboardingTasksService.listSubtasks(id, user);
  }

  // Ticking the last outstanding required subtask completes the parent
  // task in the same transaction — see markSubtaskDone.
  @UseGuards(JwtAuthGuard)
  ...
  completeSubtask(
    ...
    return this.onboardingTasksService.markSubtaskDone(subtaskId, user);
  }

  @UseGuards(JwtAuthGuard)
  @Post('subtasks/:subtaskId/reopen')
  reopenSubtask(
    @Param('subtaskId', ParseUUIDPipe) subtaskId: string,
    @CurrentUser() user: AuthenticatedUser,
  ) {
    return this.onboardingTasksService.unmarkSubtaskDone(subtaskId, user);
  }
}
