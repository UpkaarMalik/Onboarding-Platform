...
/** A template task plus the checklist that renders in its popup. Empty
 *  for tasks authored without subtasks. */
export type TemplateTaskWithSubtasks = TemplateTaskRow & {
...
export class TemplatesService {
  ...
  private async insertTasks(
    ...
    for (const task of tasks) {
      ...
      // display_order comes from array position rather than being an
      // input field, so the popup renders in the order HR authored and
      // the (template_task_id, display_order) unique constraint can't
      // be tripped by a caller reusing a number.
      let displayOrder = 1;
      for (const subtask of task.subtasks ?? []) {
        await client.query(
          `INSERT INTO template_subtasks
             (template_task_id, title, description, display_order, is_required)
           VALUES ($1, $2, $3, $4, $5)`,
          [
            rows[0].id,
            subtask.title,
  ...
  private async toTemplateWithTasks(
    ...
    // One query for every subtask in the template, grouped in memory,
    // rather than one query per task — instantiation calls this for
    // every new joinee.
    const { rows: subtaskRows } = await queryable.query<TemplateSubtaskRow>(
      `SELECT ts.* FROM template_subtasks ts
       JOIN template_tasks tt ON tt.id = ts.template_task_id
       WHERE tt.template_id = $1
       ORDER BY ts.display_order`,
      [templateId],
    );

    const subtasksByTask = new Map<string, TemplateSubtaskRow[]>();
    for (const subtask of subtaskRows) {
      const existing = subtasksByTask.get(subtask.template_task_id);
      if (existing) {
        existing.push(subtask);
      } else {
        subtasksByTask.set(subtask.template_task_id, [subtask]);
      }
    }

    const tasks: TemplateTaskWithSubtasks[] = taskRows.map((task) => ({
      ...task,
