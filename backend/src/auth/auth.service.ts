...
/** See UsersService's copy — a thing that can run a query, whether that
 *  is the pool or one transaction's client. */
interface Queryable {
...
export class AuthService {
  ...
  /** `queryable` lets the whole thing — the user row and its
   *  `user.created` log entry — join a caller's transaction, which is how
   *  POST /onboardings/joinee makes account-plus-onboarding all or
   *  nothing. Omitted, it commits on its own exactly as before. */
